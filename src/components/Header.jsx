  ...
  return (
    <header className="w-full bg-zinc-900 border-b border-zinc-800 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
      ...
      <nav className="flex items-center gap-6 text-sm text-zinc-400">
        ...
        {isLoggedIn && <Link to="/playlists" className="hover:text-white transition">Playlists</Link>}
        <button onClick={() => setAboutOpen(true)} className="hover:text-white transition cursor-pointer">About</button>
      </nav>

      <div className="flex items-center gap-4">
        {isLoggedIn ? (
          <>
            <Link to="/playlists" className="flex items-center gap-2 text-sm text-zinc-400 hover:text-white transition">
              🎵
              <span className="bg-violet-600 text-white text-xs rounded-full px-2 py-0.5 font-medium">
                {playlistCount}
              </span>
            </Link>
            <button onClick={handleLogout} className="text-sm text-zinc-400 hover:text-white transition cursor-pointer">
              Logout
            </button>
          </>
        ) : (
          <Link to="/login" className="text-sm bg-violet-600 hover:bg-violet-700 text-white px-4 py-1.5 rounded-md transition">
            Login
          </Link>
        )}
      </div>
